...
REM V8 13.2.152.41 (Electron 34) needs extra semantic patches
if "%V8_VERSION%"=="13.2.152.41" (
    echo =====[ Applying Electron 13.2.152.41 patches ]=====
    python "%WORKSPACE_DIR%\scripts\v8dasm-builders\patch-utils\apply-patch-13_2.py" "%V8_DIR%" "%PATCH_LOG%"
    if %errorlevel% neq 0 (
        echo ❌ Electron 13.2 patch failed. Build aborted.
        exit /b 1
    )
    echo ✅ Electron 13.2 patches applied
)
...
if "%V8_VERSION%"=="13.2.152.41" (
    set CLANG_DEFINES=-DV8_COMPRESS_POINTERS -DV8_ENABLE_SANDBOX
)
